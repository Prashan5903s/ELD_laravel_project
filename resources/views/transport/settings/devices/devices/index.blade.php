                                            ...
                                            @if ($permissions->contains(33))
                                                <div class="d-flex justify-content-end" data-kt_tr_u_table-toolbar="base">
                                                    ...
                                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                                        ...
                                                        onclick="window.location.href='{{ route('setting.device.create', [request()->lang]) }}'">
                                                        <i class="ki-outline ki-plus fs-2"></i>@lang('lang.add_device')
                                                    </button>
                                                    <!--end::Add user-->
                                                </div>
                                            @endif
                                            ...
                                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_tr_u_table">
                                                <thead>
                                                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                                        <th class="min-w-125px">Device type</th>
                                                        <th class="min-w-125px">Hardware</th>
                                                        <th class="min-w-125px">Serial number</th>
                                                        <th class="min-w-125px">Gateway serial</th>
                                                        <th class="min-w-125px">Gateway</th>
                                                        <th class="min-w-125px">Vechile</th>
                                                        <th class="min-w-125px">Status</th>
                                                        @if ($permissions->contains(34))
                                                ...
                                                <tbody class="text-gray-600 fw-semibold" id=userTableBody>
                                                    @foreach ($device as $value)
                                                        <tr>
                                                            <td>{{ $value->deviceType->device_type_name }}</td>
                                                            <td>{{ $value->hardware->hardware_name }}</td>
                                                            <td>{{ $value->serial_number }}</td>
                                                            <td>
                                                                {{ $value->gateway_serial }}
                                                            </td>
                                                            <td>{{ $value->gateway }}</td>
                                                            <td>
                                                                @if ($value->vehicle)
                                                                ...
                                                                <td class="text-end">
                                                                    ...
                                                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                                        ...
                                                                        <div class="menu-item px-3">
                                                                            <a href="{{ route('setting.device.edit', [request()->lang, $value->id]) }}"
