...
class DOTInspectionMobileAPIController extends Controller
{
    ...
    public function data_for_mail()
    {
        ...
        if ($auth) {
            ...
            $timeNow = Carbon::parse($currentTime)->endOfDay();
            ...
            if ($last7Days) {
                ...
                for ($i = count($last7Days) - 1; $i >= 0; $i--) {
                    ...
                    $finalData['graph_data'] = graph_hos_chart($id, $create, $last, $currentTime);
    ...
    public function dot_data($token)
    {

        $data = EmailLogs::where('sender_token', $token)
            ->select('sender_token', 'sender_id', 'start_time', 'end_time')
            ->with([
                'user:id,first_name,last_name,email,mobile_no,address,timezone',
                'userInfo:user_id,driver_id,licenseNumber,username,career_name,home_terminal_timezone,main_office_address,carrer_us_dot_number,home_terminal_name,home_terminal_address',
                'userInfo.homeTerminal:id,name,address',
                'ruleAssign:user_id,rule_id',
                'ruleAssign.rule:id,name,title',
                'vehicleAssign:driver_id,vechile_id',
                'vehicleAssign.device:vehicle_id,serial_number',
                'vehicleAssign.vehicle:id,vin,name,license_plate',
            ])->first();
        ...
        if ($data) {
            ...
            return response()->json($data_val);
        } else {

            return response()->json("No data present", 403);
        }
    }
}
